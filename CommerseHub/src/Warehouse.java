...
public class Warehouse implements InventoryManagementSystem
{
	...
	private String generate_id()
	{
		...
		boolean c=checkFile("stocks_sheet.txt");
		...
		else
		{
			try
			{
				f_rs=new FileReader("stocks_sheet.txt");
...
//			amountToRestock=Integer.parseInt(b_r.readLine());
			...
			f_os=new FileOutputStream("stocks_sheet.txt",true);
...
int amountToPick=0;
		try
		{
			while(s.equals("y"))
			{
				...
				{
					System.out.println("Only avilable stock is:'"+stock+"'");
	...
	private String findProduct(String productId) throws IOException
	...
	public RestockingResult restockProduct(String productId, int amountToRestock)
	{
		...
		try
		{
			...
			f_os=new FileOutputStream("stocks_sheet.txt",false);
			p_os=new PrintStream(f_os);
	...
	public static void main(String args[]) throws IOException
	{
		...
		System.out.println("3. View Product Lists");
		System.out.println("---------------------------------");
